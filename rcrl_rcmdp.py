...
log_file = "rcrl_training_log_episode_betaexp_normalizedcost_eps0.5_beta_1_200.log"
plot_file = "training_metrics_episode_betaexp_normalizedcost_eps0.5_beta_1_200.png"
...
def robust_value_function(trajectory, actor, cost_critic, gamma):
    """
    Compute the robust value function V_h^pi(s) and vl_pi for the episode.
    ...
    Returns:
        ...
        vl_pi (float): Maximum cost value (max(V_h(s))) over the episode.
    """
    ...
    for state, action, next_state, h_s in zip(states, actions, next_states, costs):
        ...
        all_actions = torch.arange(len(probs))

        # Compute Q_h(s, a) for all actions
        q_values = []
        for a in all_actions:
            # Compute h(s) for the current state
            if a == action:  # Use the actual action's cost from the trajectory
                h_s = h_s
            else:
                h_s = compute_cost(state, safe_distance, safe_angle)

            # Get V_h(next_state) from the cost critic
...
def main():
    ...
    for ep in range(epochs):
        ...
        while not done:
            ...
            # h_s=0
            # Compute robust value function and safety indicator
            trajectory['states'].append(state)  # Track states for the episode
