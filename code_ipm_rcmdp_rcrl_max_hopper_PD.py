    ...
    def update(self, replay_buffer, total_steps):
        ...
        for _ in range(self.K_epochs):
            ...
            with torch.no_grad():  # adv and v_target have no gradient
                ...
                # ---------------------------------------------------------
                # Compute cumulative episode cost from replay-buffer cost
                # ---------------------------------------------------------

                cost_np = c.detach().flatten().cpu().numpy()
                done_np = done.detach().flatten().cpu().numpy()

                episode_cost_sums = []
                running_sum = 0.0
                has_partial_episode = False

                for cost_t, done_t in zip(cost_np, done_np):
                    running_sum += float(cost_t)
                    has_partial_episode = True
                ...
                # Include partial trajectory if batch ends before done=True
                if has_partial_episode:
                    episode_cost_sums.append(running_sum)
