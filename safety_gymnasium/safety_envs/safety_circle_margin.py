...
class SafetyCircleMargin(gym.Wrapper):
    ...
    """
    def __init__(self, env: gym.Env, safety_clearance: float = 0.20):
        super().__init__(env)
    ...
    def _margin_from_obs(self, obs: np.ndarray, use_lidar=False) -> float:
        ...
        for wall_pos in wall_positions:
            ...
            # For Circle environment, sigwalls are vertical/horizontal barriers
            # We need to calculate distance to the line segment, not just the center point
            
            # Get the wall geometry parameters
            wall_size = sigwalls.size  # This is the half-length of the wall
            locate_factor = sigwalls.locate_factor  # Distance from origin
