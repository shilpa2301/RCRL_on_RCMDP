...
#shilpa optimality sanity
def set_input_upper_bound(env, upper, constraint_idx=0):
    """
    Dynamically update upper bound for a bounded input constraint.

    Supports both:
        upper_bounds: [0.12]
    and:
        upper_bounds: [0.12, 0.12]
    ...
    if not hasattr(env, "constraints") or env.constraints is None:
        raise ValueError("env.constraints is None. No constraints found.")

    con = env.constraints.constraints[constraint_idx]

    if not hasattr(con, "upper_bounds"):
        raise AttributeError(
            "Selected constraint does not have upper_bounds. "
            "Make sure constraint_idx points to a BoundedConstraint."
        )

    # If upper is scalar, expand to same shape as existing upper_bounds.
    if np.isscalar(upper):
        upper_vec = np.full_like(con.upper_bounds, float(upper), dtype=np.float32)
    else:
        upper_vec = np.asarray(upper, dtype=np.float32).reshape(-1)

    assert upper_vec.shape == con.upper_bounds.shape, (
        f"Expected upper shape {con.upper_bounds.shape}, got {upper_vec.shape}"
    )

    con.upper_bounds = upper_vec

    # BoundedConstraint represents:
    # g_lower = -x + lower
    # g_upper =  x - upper
    # with b = [-lower, upper]
    con.b = np.hstack((-con.lower_bounds, con.upper_bounds)).astype(np.float32)

    # Refresh function explicitly.
    # Important: con=con avoids Python late-binding issues.
    con.sym_func = lambda x, con=con: con.A @ con.constraint_filter @ x - con.b


def input_upper_bound_schedule(episode):
    """
    Increase input upper bound by 0.01 every 5000 episodes,
    starting at 0.12 and capped at 0.17.

    episode 0-4999       -> 0.12
    episode 5000-9999    -> 0.13
    episode 10000-14999  -> 0.14
    episode 15000-19999  -> 0.15
    episode 20000-24999  -> 0.16
    episode >= 25000     -> 0.17
    """

    upper = 0.12 + 0.01 * (episode // 5000)
    upper = min(upper, 0.17)
    return upper


def apply_constraint_schedule(args, episode, env, env_evaluate=None, env_reset=None):
    """
    Apply scheduled input upper bound to training/eval/reset environments.
    """

    if args.env != "Quadrotor":
        return None

    new_upper = input_upper_bound_schedule(episode)

    set_input_upper_bound(env, new_upper)

    if env_evaluate is not None:
        set_input_upper_bound(env_evaluate, new_upper)

    if env_reset is not None:
        set_input_upper_bound(env_reset, new_upper)
...
def main(args, run_number):
    ...
    for total_steps in tqdm(range(args.max_train_steps)):
        # ------------------------------------------------------------
        # Dynamic constraint schedule for quadrotor input upper bounds.
        # NOTE: total_steps is actually episode index in this code,
        # because env.reset() is called once per outer-loop iteration.
        # ------------------------------------------------------------
        current_upper_bound = apply_constraint_schedule(
            args,
            total_steps,
            env,
        ...
        if args.env == "Quadrotor" and total_steps % 5000 == 0:
            con = env.constraints.constraints[0]
            print(
                f"\n[Constraint Schedule] Episode {total_steps} | "
                f"upper_bounds = {con.upper_bounds} | "
                f"b = {con.b}\n"
            )
        s, _ = env.reset()#[0][0]
        ...
        if total_steps % args.evaluate_freq == 0:
            ...
            if args.env == "Quadrotor":
                current_ub = env.constraints.constraints[0].upper_bounds
            else:
                current_ub = None

            print(
                f"evaluate_num:{evaluate_num} \t "
                f"episode:{total_steps} \t "
                f"upper_bounds:{current_ub} \t "
                f"reward:{evaluate_reward:.3f} \t "
                f"{cost_str} \t "
                f"Total Cost:{evaluate_total_cost:.3f} \t "
                f"{max_cost_str} \t "
