...
class Robust_RCAC_NPG:
    ...
    def update(self, replay_buffer, total_steps):
        ...
        # Make sure dual_lambda is vector and on correct device
        ...
        # If somehow scalar, expand to cost_dim
        ...
        # c should be [batch_size, cost_dim]
        ...
        for _ in range(self.K_epochs):
            ...
            with torch.no_grad():
                ...
                # Critic predictions
                # ====================================================
                vs = self.Rcritic(s)       # shape [B, 1]
                vs_ = self.Rcritic(s_)     # shape [B, 1]

                vcs = self.Ccritic(s)      # shape [B, cost_dim]
                vcs_ = self.Ccritic(s_)    # shape [B, cost_dim]

                # ====================================================
                # Multi-constraint peak value
                # ====================================================
                # Peak value for each cost dimension
                # shape [cost_dim]
                vl_pi_each_cost = vcs.max(dim=0).values

                # Violation for each constraint
                # shape [cost_dim]
                constraint_violation_each = vl_pi_each_cost - eps_vec

                # Overall worst violation
                # scalar
                worst_violation = constraint_violation_each.max()

                # Overall peak-cost value
                # scalar
                vl_pi = vl_pi_each_cost.max()

                # ====================================================
                # Vector dual update
                # lambda_i <- [lambda_i + lr * violation_i]_+
                # ====================================================
                ...
                # ====================================================
                # Cost critic target
                # Multi-constraint robust/peak-style backup
                # ====================================================
                ...
                # Bootstrapped next cost values
                # shape [B, cost_dim]
                bootstrapped_cost = not_dw * vcs_

                # Elementwise smooth max between immediate cost and next cost
                # shape [B, cost_dim]
                ...
                reg_norm_c = torch.norm(last_weight_c, p=2, dim=1)  # shape [cost_dim]
                reg_norm_c = reg_norm_c.view(1, -1).expand_as(vcs)  # shape [B, cost_dim]

                # Cost critic target
                # shape [B, cost_dim]
                v_target_c = cost_deltas + vcs + self.weight_reg * reg_norm_c

                # For actor, keep multi-dimensional cost advantage
                # shape [B, cost_dim]
                ...
                # ====================================================
                # Multi-constraint primal-dual advantage
                #
                # A_PD = A_r - sum_i lambda_i A_c_i
                # ====================================================
                lambda_vec = self.dual_lambda.detach().view(1, -1)  # shape [1, cost_dim]

                cost_penalty_adv = (adv_c * lambda_vec).sum(
                    dim=1,
                    keepdim=True
                )  # shape [B, 1]
                ...
                # Optional normalization of combined advantage
                if self.use_adv_norm:
