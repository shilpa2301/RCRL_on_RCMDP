...
class ReplayBuffer:
    ...
    def numpy_to_tensor(self, device):
        ...
        a_logprob = torch.tensor(self.a_logprob, dtype=torch.float32, device=device)
        r = torch.tensor(self.r, dtype=torch.float32, device=device)
        c = torch.tensor(self.c, dtype=torch.float32, device=device)
        s_ = torch.tensor(self.s_, dtype=torch.float32, device=device)
        dw = torch.tensor(self.dw, dtype=torch.float32, device=device)
...
class RPCRL:
    ...
    #     return a
    def evaluate(self, s):
        # Environment gives numpy array on CPU.
        # Convert to torch tensor on selected device.
        s = torch.unsqueeze(
    ...
    def choose_action(self, s):
        ...
        # Important: env.step needs numpy on CPU.
        return (
    ...
    def log_sum_exp_fn(self, a, b, eta=0.01):
        # GPU-safe stable log-sum-exp approximation of max(a, b)
    ...
    def update(self, replay_buffer, total_steps):
        ...
        s, a, a_logprob, r, c, s_, dw, done = (
            replay_buffer.numpy_to_tensor(self.device)
        )  # Get training data
        """
        ...
        for _ in range(self.K_epochs):
            ...
            with torch.no_grad():  # adv and v_target have no gradient
                ...
                # else:
                #     ch = 0
                beta_penalty_item = beta_penalty.item()

                if self.warm_start_flag == 1:
                    ch = np.argmax([vs_mean, beta_penalty_item])
                else:
                    ch = 0
                ...
                # ------------------------------------------------------------------
                # GAE-style cost advantage using log-sum-exp instead of max
                # ------------------------------------------------------------------

                costs_flat = c.view(-1)
                vcs_flat = vcs.view(-1)
                vcs_next_flat = vcs_.view(-1)
                done_flat = done.view(-1)
                dw_flat = dw.view(-1)

                T = costs_flat.shape[0]
                adv_c_list = []
                ...
                for t in range(T):
                    ...
                    for k in range(1, M + 1):
                        ...
                        if k > 1:
                            if done_flat[t:end_idx - 1].sum() > 0:
                                break

                        # c_t, ..., c_{t+k-1}
                        cost_window = costs_flat[t:end_idx]

                        # V_c(s_{t+k}) - V_c(s_t)
                        if end_idx < T:
                            bootstrap_value = vcs_flat[end_idx] * (1.0 - done_flat[end_idx - 1])
                        else:
                            bootstrap_value = vcs_next_flat[end_idx - 1] * (1.0 - dw_flat[end_idx - 1])

                        bootstrap_adv = bootstrap_value - vcs_flat[t]

                        # Smooth max with log-sum-exp
                        terms = torch.cat([
                            cost_window,
                            bootstrap_adv.view(1),
                        ])

                        A_t_k = tau_lse * torch.logsumexp(terms / tau_lse, dim=0)

                        weight = self.lamda ** (k - 1)

                        weighted_sum = weighted_sum + weight * A_t_k
                        weight_sum = weight_sum + weight

                    adv_t = weighted_sum / (weight_sum + eps)
                    adv_c_list.append(adv_t)

                adv_c = torch.stack(adv_c_list).view(-1, 1)

                v_target_c = adv_c + vcs + self.weight_reg * reg_norm
