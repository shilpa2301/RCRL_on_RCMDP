...
class PrimalDual:
  ...
  def update(self, replay_buffer, total_steps):
            ...
            adv_combined = adv_r - self.lambda_ * adv_c
            ...
            for _ in range(self.K_epochs):
              ...
              for index in BatchSampler(SubsetRandomSampler(range(self.batch_size)), self.mini_batch_size, False):
                ...
                log_probs = dist_now.log_prob(a[index]).sum(dim=1, keepdim=True)
                # a/b=exp(log(a)-log(b))  In multi-dimensional continuous action space，we need to sum up the log_prob
                # ratios = torch.exp(a_logprob_now.sum(1, keepdim=True) - a_logprob[index].sum(1,
                ratios = torch.exp(log_probs - a_logprob_old_sum[index])

                surr1 = ratios * adv_combined[index]  # Only calculate the gradient of 'a_logprob_now' in ratios
                surr2 = torch.clamp(ratios, 1 - self.epsilon, 1 + self.epsilon) * adv_combined[index]
                actor_loss = (-torch.min(surr1, surr2).mean() - self.entropy_coef * dist_entropy.mean() ) # Trick 5: policy entropy
                # Update actor
            ...
            # print("loss actor, reward critic=",actor_loss.item(),reward_critic_loss.item() )

            # print("advr, adv c=", adv_r, adv_r)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--warm_start_episode",type=int,default=500,help="warm_start_episode") 
