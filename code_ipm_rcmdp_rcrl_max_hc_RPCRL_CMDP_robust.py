...
from envs.half_cheetah import HalfCheetahCMDP, HalfCheetahCMDPPerturbed
...
class ReplayBuffer:
    def __init__(self, args):
        ...
        self.incremental_max_cost = np.zeros((args.batch_size, 1), dtype=np.float32)
        self.count = 0

    def store(self, s, a, a_logprob, r, c, s_, dw, done, incremental_max_cost):
        self.s[self.count] = s
        self.a[self.count] = a
        self.a_logprob[self.count] = a_logprob
        self.r[self.count] = r
        ...
        self.dw[self.count] = dw
        self.done[self.count] = done
        self.incremental_max_cost[self.count] = incremental_max_cost
        self.count += 1
    ...
    def numpy_to_tensor(self, device):
        ...
        incremental_max_cost = torch.as_tensor(self.incremental_max_cost, dtype=torch.float32, device=device)
        return s, a, a_logprob, r, c, s_, dw, done, incremental_max_cost
...
class PrimalDual:
    def __init__(self, args):
        ...
        elif args.env == "HalfCheetahCMDPPerturbed":
            self.env = HalfCheetahCMDPPerturbed(dense_cost_weight=args.dense_cost_weight, cost_scale=args.cost_scale, sigma_gravity=args.sigma_gravity)
        else:
        ...
        #shilpa robust cmdp
        # Augmented observation layout for HalfCheetahCMDPPerturbed:
        # obs = [qpos(9), qvel(9), max_cost_memory(1)]
        self.physical_dim = 18
        self.m_index = 18

        # Whether the memory coordinate in obs is scaled as cost_scale * max_cost
        self.memory_is_scaled = True  # set False if you change env to append raw max_cost

        # Weight for memory sensitivity in augmented Lipschitz penalty
        # 0.0 = physical-only robustness
        # 1.0 = full augmented-state robustness
        self.m_lip_weight = getattr(args, "m_lip_weight", 0.01) #0.1)

        # Whether to use input-gradient Lipschitz regularization
        self.use_input_lip_reg = getattr(args, "use_input_lip_reg", True)
    ...
    #shilpa robust cmdp
    def critic_split_lipschitz(
        self,
        critic,
        states,
        physical_dim,
        m_index,
        memory_is_scaled=True,
    ):
        """
        Estimate:
            L_s = || dV / d physical_state ||
            L_M = | dV / d raw_max_cost |

        If env observation contains z = cost_scale * M, autograd gives dV/dz.
        We convert to raw M sensitivity:
            dV/dM = cost_scale * dV/dz
        """

        states_req = states.detach().clone().requires_grad_(True)

        values = critic(states_req)  # [batch, 1]
        ...
        # Physical gradient: dV/ds
        grad_s = grads[:, :physical_dim]

        # Memory gradient wrt network input z
        grad_m_input = grads[:, m_index:m_index + 1]

        # If input is z = cost_scale * M, convert dV/dz to dV/dM
        if memory_is_scaled:
            grad_m_raw = self.cost_scale * grad_m_input
        else:
            grad_m_raw = grad_m_input

        # Per-sample norms
        Ls_per_sample = torch.sqrt(torch.sum(grad_s ** 2, dim=1, keepdim=True) + 1e-8)
        Lm_per_sample = torch.abs(grad_m_raw)

        # Batch averages
        Ls = Ls_per_sample.mean()
        Lm = Lm_per_sample.mean()
    ...
    def choose_action(self, s):
        ...
        if self.policy_dist == "Beta":
            ...
            with torch.no_grad():
                pre_tanh_a, env_a, a_logprob = self.actor.sample_with_pre_tanh(s)

            return (
                pre_tanh_a.detach().cpu().numpy().flatten(),   # store this in buffer
                env_a.detach().cpu().numpy().flatten(),        # send this to environment
                a_logprob.detach().cpu().numpy().flatten(),    # scalar log prob
    )
        else:
            with torch.no_grad():
                dist = self.actor.get_dist(s)
    ...
    def update(self, replay_buffer, total_steps):
        ...
        s, a, a_logprob, r, c, s_, dw, done, incremental_max_cost = replay_buffer.numpy_to_tensor(self.device)
...
def main(args, run_number):
    ...
    if args.env == "HalfCheetahCMDP":
        ...
        env = (HalfCheetahCMDPPerturbed(dense_cost_weight=args.dense_cost_weight, cost_scale=args.cost_scale, sigma_gravity=args.sigma_gravity))
        env_evaluate = (HalfCheetahCMDPPerturbed(dense_cost_weight=args.dense_cost_weight, cost_scale=args.cost_scale, sigma_gravity=args.sigma_gravity))
        env_reset = HalfCheetahCMDPPerturbed(dense_cost_weight=args.dense_cost_weight, cost_scale=args.cost_scale, sigma_gravity=args.sigma_gravity)
    # Set random seed
    # env.reset(seed=seed)
    # env.seed(seed)
    env.reset(seed=seed)
    ...
    for total_steps in tqdm(range(args.max_train_steps)):
        ...
        while not done:
            ...
            # replay_buffer.store(s, a, a_logprob, r + reward_offset, c, s_, dw, done, incremental_max_cost)

            replay_buffer.store(s,buffer_a, a_logprob,r + reward_offset,c,s_,dw,done, incremental_max_cost)
            # s = copy.deepcopy(s_)
            s = s_.copy()
