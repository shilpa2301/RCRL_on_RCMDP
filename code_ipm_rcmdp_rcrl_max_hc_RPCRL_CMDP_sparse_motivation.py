...
from envs.half_cheetah import HalfCheetahCMDPSparse, HalfCheetahCMDPPerturbed
...
class PrimalDual:
    def __init__(self, args):
        if args.env == "HalfCheetahCMDPSparse":
            self.env = HalfCheetahCMDPSparse(dense_cost_weight=args.dense_cost_weight, cost_scale=args.cost_scale)
...
def main(args, run_number):
    ...
    if args.env == "HalfCheetahCMDPSparse":
        env = (HalfCheetahCMDPSparse(dense_cost_weight=args.dense_cost_weight, cost_scale=args.cost_scale))
        env_evaluate = (HalfCheetahCMDPSparse(dense_cost_weight=args.dense_cost_weight, cost_scale=args.cost_scale))
        env_reset = HalfCheetahCMDPSparse(dense_cost_weight=args.dense_cost_weight, cost_scale=args.cost_scale)
    elif args.env == "HalfCheetahCMDPPerturbed":
    ...
    # Tracking sparsity of received raw costs c
    episode_cost_event_counts = []
    episode_cost_event_rates = []
    ...
    for total_steps in tqdm(range(args.max_train_steps)):
        ...
        while not done:
            ...
            # ------------------------------------------------------------
            # Sparse cost tracking using raw environment cost c
            # ------------------------------------------------------------
            if np.any(np.asarray(c) > cost_event_tol):
                cost_event_count += 1
        ...
        # ------------------------------------------------------------
        # Save sparse cost statistics for this episode
        # ------------------------------------------------------------
        episode_total_steps = max(episode_steps, 1)
        cost_event_rate = cost_event_count / episode_total_steps

        episode_cost_event_counts.append(cost_event_count)
        episode_cost_event_rates.append(cost_event_rate)
        episode_total_steps_list.append(episode_total_steps)
        ...
        np.save(
            f"{plot_data_dir}/episode_costs.npy",
            np.asarray(episode_costs),
        )

        np.save(
            f"{plot_data_dir}/episode_cost_event_counts.npy",
            np.asarray(episode_cost_event_counts),
        )
        ...
        np.save(
            ...
            np.asarray(episode_total_steps_list),
        )
        ...
        plot_cost_sparsity(
            ...
            episode_total_steps_list,
            save=True,
            filename=f"{plot_data_dir}/cost_sparsity.png",
        )
