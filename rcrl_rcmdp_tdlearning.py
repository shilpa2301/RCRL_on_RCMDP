...
log_file = "rcrl_training_log_td_learning_betaexp_eg.log"
plot_file = "training_metrics_td_learning_betaexp_eg.png"
...
def main():
    ...
    for ep in range(epochs):
        ...
        while not done:
            ...
            #     action = torch.tensor(env.action_space.sample())  # Take a random action
            # else:
            #     # action = dist.sample()  # Sample action from policy
            #     action = torch.argmax(probs)  # Selects the action with the highest probability
            ...
            # advantage = reward_target - reward_value.item()

            # # Actor loss
            # # actor_loss = -dist.log_prob(action) * advantage
            # entropy = -torch.sum(probs * torch.log(probs + 1e-8))  # Compute entropy
            # actor_loss = -(dist.log_prob(action) * advantage + 0.01 * entropy)  # Add entropy term
