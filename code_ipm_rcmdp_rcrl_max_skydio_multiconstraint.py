...
def evaluate_policy(args, env, agent, state_norm=None, reward_scaling=None):
    ...
    for _ in range(times):
        ...
        while not done:
            ...
            c = args.cost_scale * (np.asarray(c, dtype=np.float32).reshape(-1))
...
def main(args, run_number):
    ...
    if args.env == "CartPolePerturbedEnv":
        ...
        env = SkydioTrackingMultiConstraintEnv()
        env_evaluate = SkydioTrackingMultiConstraintEnv()
        env_reset = SkydioTrackingMultiConstraintEnv()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--cost_dim", type=int, default=8, help="dimension of the cost"
    )
    parser.add_argument(
