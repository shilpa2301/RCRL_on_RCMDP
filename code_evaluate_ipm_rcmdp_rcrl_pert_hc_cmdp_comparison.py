...
import glob
from envs.half_cheetah import HalfCheetahWithPos, HalfCheetahWithPosPerturbed, HalfCheetahCMDPPerturbed, HalfCheetahWithPosPerturbedTest, HalfCheetahCMDPPerturbedTest
...
def test_single_models(args, model_specs, perturbation_stds, num_episodes=100):
    ...
    for std in perturbation_stds:
        # Sample one perturbation value for this std.
        # This same value is used by both CMDP and non-CMDP envs.
        ...
        for spec in model_specs:
            ...
            if is_cmdp:
                env = HalfCheetahCMDPPerturbedTest(
                    sigma_gravity=std,
                    shared_gravity_perturbation=shared_gravity_perturbation,
                    seed=args.seed,
                )
            else:
                env = HalfCheetahWithPosPerturbedTest(
                    sigma_gravity=std,
                    shared_gravity_perturbation=shared_gravity_perturbation,
                    seed=args.seed,
                )
...
if __name__ == "__main__":
    ...
    model_specs = [
        {
            "label": "Surrogate Obj(NP)",
            "model_path": "./models/HalfCheetahWithPos/run2/Best_RCAC",
            "env_type": "pos"
        },
        {
            "label": "Ours",
    ...
    plot_evaluation_single(
        ...
        color_map=color_map,
        save=True,
        base_filename="plot_inference/HC_comparison_CMDProbust",
        smooth_window=20
    )
