...
def robust_value_function(state, actor, cost_critic, perturb_eps, safe_distance):
...
def main():
    ...
    for ep in range(episodes):
        ...
        while not done:
            ...
            v_h_pi, h_s = robust_value_function(state, actor, cost_critic, perturb_eps, safe_distance)
        ...
        # Display
        if (ep + 1) % 50 == 0:
            print(f"Ep {ep+1} | Reward: {total_reward:.1f} | Cost: {total_cost:.2f} | Actor Loss: {actor_loss.item():.3f} | Best Reward (under safety): {best_reward:.1f}")
