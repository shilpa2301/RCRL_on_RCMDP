...
from envs.pusher import PusherWithCost, PusherWithCostPerturbed
...
class RPCRL:
    def __init__(self, args):
        ...
        if args.env == "PusherWithCost":
            self.env = PusherWithCost(sigma_viscosity=0.0, max_steps=100)
        elif args.env == "PusherWithCostPerturbed":
            self.env = PusherWithCostPerturbed(sigma_viscosity=args.sigma_viscosity, max_steps=100)
...
def main(args, run_number):
    ...
    if args.env == "PusherWithCost":
        env = (PusherWithCost(sigma_viscosity=0.0, max_steps=100))  
        env_evaluate = (PusherWithCost(sigma_viscosity=0.0, max_steps=100))  
        env_reset = PusherWithCost(sigma_viscosity=0.0, max_steps=100)
    elif args.env == "PusherWithCostPerturbed":
        env = (PusherWithCostPerturbed(sigma_viscosity=args.sigma_viscosity, max_steps=100))  
        env_evaluate = (PusherWithCostPerturbed(sigma_viscosity=args.sigma_viscosity, max_steps=100))  
        env_reset = PusherWithCostPerturbed(sigma_viscosity=args.sigma_viscosity, max_steps=100)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="PusherWithCost",
        help="PusherWithCost/PusherWithCostPerturbed",
    )
