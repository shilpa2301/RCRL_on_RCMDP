...
from envs.half_cheetah_multi_constraint_cost import (
    ...
    HalfCheetahMultiConstraintCostPerturbed,
    HalfCheetahMultiConstraintCostPerturbedTest,
    MULTI_CONSTRAINT_COST_VERSION,
    PERTURBED_MULTI_CONSTRAINT_COST_VERSION,
    COST_NAMES,
)
...
class Robust_RCAC_NPG:
    def __init__(self, args):
        ...
        elif args.env == "HalfCheetahMultiConstraintCostPerturbed":
            self.env = HalfCheetahMultiConstraintCostPerturbed(
    ...
    def update(self, replay_buffer, total_steps):
        ...
        for _ in range(self.K_epochs):
            ...
            with torch.no_grad():  # adv and v_target have no gradient
                ...
                #shilpa multi constraint
                # vl_pi = vcs.max()
                vl_pi_each_cost = vcs.max(dim=0).values  # shape [2]
                vl_pi = vl_pi_each_cost.max()            # scalar
                ...
                last_weight = linear_layers[-1].weight  # shape: [2, hidden_width]

                # One L2 norm per cost output
                ...
                # Make dw broadcastable from [batch_size, 1] to [batch_size, 2]
                not_dw = 1.0 - dw
                if not_dw.shape != vcs_.shape:
                    not_dw = not_dw.expand_as(vcs_)

                # Bootstrapped next cost values, shape [batch_size, 2]
                bootstrapped_cost = not_dw * vcs_

                # Step 1:
                # Elementwise log-sum-exp between corresponding dimensions:
                # cost_backup[:, 0] = LSE(c[:, 0], bootstrapped_cost[:, 0])
                # cost_backup[:, 1] = LSE(c[:, 1], bootstrapped_cost[:, 1])
                ...
                # Cost critic target remains 2-dimensional
                # Shape: [batch_size, 2]
                v_target_c = cost_deltas + vcs + self.weight_reg * reg_norm

                # Step 2:
                # Smooth max over the two cost dimensions.
                # Shape: [batch_size, 1]
                adv_c = self.log_sum_exp_dim(cost_deltas, dim=1, eta=0.01, keepdim=True)
...
def main(args, run_number):
    ...
    if args.env == "HalfCheetahMultiConstraintCost":
        ...
        env = HalfCheetahMultiConstraintCostPerturbed(
            sigma_gravity=args.sigma_gravity,
            max_steps=args.max_episode_steps,
            perturb_each_step=True,
        )
        env_evaluate = HalfCheetahMultiConstraintCostPerturbed(
            sigma_gravity=args.sigma_gravity,
            max_steps=args.max_episode_steps,
            perturb_each_step=True,
        )
        env_reset = HalfCheetahMultiConstraintCostPerturbed(
            sigma_gravity=args.sigma_gravity,
            max_steps=args.max_episode_steps,
            perturb_each_step=True,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        choices=["HalfCheetahMultiConstraintCost", "HalfCheetahMultiConstraintCostPerturbed"],
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.7,
        help="Standard deviation for gravity perturbation",
    )
    ...
    parser.add_argument(
        ...
        type=bool,
        default=True,
        help="If True, perturb gravity every step. If False, perturb once per episode.",
    )
