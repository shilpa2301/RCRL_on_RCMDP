...
from envs.hopper3 import HopperPerturbedEnv
...
class HighestCostCritic(nn.Module):
    def __init__(self, args):
        super(HighestCostCritic, self).__init__()
        self.fc1 = nn.Linear(args.state_dim, args.hidden_width)
        self.fc2 = nn.Linear(args.hidden_width, args.hidden_width)
        self.fc3 = nn.Linear(args.hidden_width, 1)
        self.activate_func = nn.ReLU()
        ...
        if args.use_orthogonal_init:
            print("------use_orthogonal_init for HighestCostCritic------")
            orthogonal_init(self.fc1)
            orthogonal_init(self.fc2)
            orthogonal_init(self.fc3)
...
class Robust_RCAC_NPG:
  def __init__(self,args):
    ...
    self.HighestCostCritic = HighestCostCritic(args)
    ...
    if self.set_adam_eps:  # Trick 9: set Adam epsilon=1e-5
        ...
        self.optimizer_HighestCostCritic = torch.optim.Adam(self.HighestCostCritic.parameters(), lr=self.lr_c, eps=1e-5)
    else:
  ...
  def persistent_safety_function(self, trajectory, actor, highest_cost_critic, gamma):
    ...
    for i in range(len(states)):
        ...
        for sampled_action in sampled_actions:
            ...
            next_value = highest_cost_critic(simulated_next_state)#.item()

            # Compute Q_h(s, a)
            # print("current_cost, next_value=", current_cost, next_value)
  ...
  def update(self, replay_buffer, total_steps):
        ...
        with torch.no_grad():  # adv and v_target have no gradient
            ...
            vhcs = self.HighestCostCritic(s)  # Use the HighestCostCritic
            vhcs_ = self.HighestCostCritic(s_)
            # IPM uncertainty set
            ...
            with torch.no_grad():
                ...
                vl_pi = self.persistent_safety_function(trajectory, self.actor, self.HighestCostCritic, self.gamma)
        ...
        for _ in range(self.K_epochs):
            ...
            for index in BatchSampler(SubsetRandomSampler(range(self.batch_size)), self.mini_batch_size, False):
                ...
                v_hcs = self.HighestCostCritic(s[index])  # Update HighestCostCritic

                # Calculate the loss of critic
                Rcritic_loss = F.mse_loss(v_target[index], v_s)
                Ccritic_loss = F.mse_loss(v_target[index], v_cs)
                HighestCostCritic_loss = F.mse_loss(v_target[index], v_hcs)  # Loss for HighestCostCritic
                ...
                # Update HighestCostCritic
                self.optimizer_HighestCostCritic.zero_grad()
                HighestCostCritic_loss.backward()
                if self.use_grad_clip:
                    torch.nn.utils.clip_grad_norm_(self.HighestCostCritic.parameters(), 0.5)
                self.optimizer_HighestCostCritic.step()
...
def main(args, run_number):
    ...
    for total_steps in tqdm(range(args.max_train_steps)):
        ...
        while not done:
            ...
            #     buffer_flag = 1
            # if  buffer_flag==1 and total_steps % args.evaluate_freq:
...
if __name__ == '__main__':
    ...
    # parser.add_argument("--buffer_flag",type=int,default=0,help="Buffer_flag") 
