...
def plot_mean_std_multiple_dirs(data_dirs, labels, num_runs_list, save=False, base_filename="aggregated_plot", smooth_window=10):
    ...
    """
    baseline_cost_threshold = 1.0  # Set the baseline cost threshold
    ...
    for data_dir, label, num_runs in zip(data_dirs, labels, num_runs_list):
        ...
        all_rewards = np.array([r[:5500] for r in all_rewards])
    ...
    for data_dir, label, num_runs in zip(data_dirs, labels, num_runs_list):
        ...
        all_max_costs = np.array([c[:5500] for c in all_max_costs])
...
data_dirs = [
    "./plot_data/HumanoidWithCost",
    "./plot_data/HumanoidWithCostPerturbed",
    # "./plot_data/CartPolePerturbedEnv"
]
...
plot_mean_std_multiple_dirs(data_dirs, labels, num_runs_list, save=True, base_filename="train_plots/humanoid_training", smooth_window=80)
