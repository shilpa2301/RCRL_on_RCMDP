    ...
    def update(self, replay_buffer, total_steps):
        ...
        for _ in range(self.K_epochs):
            ...
            with torch.no_grad():  # adv and v_target have no gradient
                ...
                if ch == 1:
                    ...
                    adv = cost_deltas.clone()  # already correct

                    # ── v_target for cost critic ─────────────────────────────────────────
                    v_target = cost_deltas + vcs #+ self.alpha * a_logprob.sum(dim=1, keepdim=True)
        ...
        # for _ in range(self.K_epochs):
            ...
            for index in BatchSampler(
                ...
                # with torch.no_grad():
                #     # Fresh next-state values from the CURRENT critic
                #     vcs_mb_next = self.Ccritic(s_[index])
                #     # LSE Bellman target is now always consistent with current critic
                #     v_target_mb = (
                #         (1 - self.gamma) * c[index]
                #         + self.gamma * self.log_sum_exp_fn(
                #             c[index], (1.0 - dw[index]) * vcs_mb_next
                #         )
                #     )   # shape: (mini_batch_size, 1)

                # v_cs = self.Ccritic(s[index])
                # Ccritic_loss = F.mse_loss(v_cs, v_target_mb)
