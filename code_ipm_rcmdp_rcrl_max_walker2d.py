...
from envs.walker2d import Walker2dWithCost, Walker2dWithCostPerturbed
...
class Robust_RCAC_NPG:
    def __init__(self, args):
        if args.env == "CartPolePerturbedEnv":
            ...
            self.env = SwimmerWithPosPerturbed(floor_friction=0.0, max_steps=1000)
        elif args.env == "HumanoidWithCost":
            self.env = HumanoidWithCost()
        elif args.env == "HumanoidWithCostPerturbed":
            self.env = HumanoidWithCostPerturbed(floor_friction=args.floor_friction, max_steps=500)
        elif args.env == "Walker2dWithCost":
            self.env = Walker2dWithCost(sigma_gravity=0.0, max_steps=500)
        elif args.env == "Walker2dWithCostPerturbed":
            self.env = Walker2dWithCostPerturbed(sigma_gravity=0.7, max_steps=500)
        else:
...
def main(args, run_number):
    ...
    if args.env == "CartPolePerturbedEnv":
        ...
        env = ( HumanoidWithCostPerturbed(floor_friction=args.floor_friction, max_steps=500) )
        env_evaluate = ( HumanoidWithCostPerturbed(floor_friction=args.floor_friction, max_steps=500) )
        env_reset = HumanoidWithCostPerturbed(floor_friction=args.floor_friction, max_steps=500)
    elif args.env == "Walker2dWithCost":
        env = Walker2dWithCost()
        env_evaluate = Walker2dWithCost()
        env_reset = Walker2dWithCost()
    elif args.env == "Walker2dWithCostPerturbed":
        env = Walker2dWithCostPerturbed(sigma_gravity=args.sigma_gravity, max_steps=500)
        env_evaluate = Walker2dWithCostPerturbed(sigma_gravity=args.sigma_gravity, max_steps=500)
        env_reset = Walker2dWithCostPerturbed(sigma_gravity=args.sigma_gravity, max_steps=500)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="Walker2dWithCostPerturbed",
