...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim):
        ...
        self.log_std = nn.Parameter(torch.zeros(1, action_dim))  # shape: (1, action_dim)

    def forward(self, s):
        return self.net(s)  # Returns mean

    def get_dist(self, s):
        mean = self.forward(s)
        log_std = self.log_std.expand(mean.shape[0], -1)  # Expand to (batch_size, action_dim)
...
import matplotlib
matplotlib.use('Agg')  # Add at the TOP of your script, before importing pyplot
import matplotlib.pyplot as plt

def plot_metrics(rewards, max_costs, costs, baseline=None, save=True, filename="training_metrics.png"):
    """
    Plot training metrics (reward, max cost, episodic cost) over episodes
    and save the plot to file.
    Args:
        rewards:    List of total rewards per episode.
        max_costs:  List of max costs per episode.
        costs:      List of total costs per episode.
        baseline:   Cost constraint threshold — drawn as a horizontal reference line.
    ...
    axes[0].plot(episodes, rewards, color="blue", label="Episode Reward")
    ...
    # ── Subplot 2: Max Cost ───────────────────────────────────────────────────
    axes[1].plot(episodes, max_costs, color="orange", label="Episode Max Cost")
    if baseline is not None:
        axes[1].axhline(
            ...
            linestyle="--",
            linewidth=1.5,
            label=f"Constraint threshold ({baseline})",
        )
    ...
    # ── Subplot 3: Episodic Cost ──────────────────────────────────────────────
    axes[2].plot(episodes, costs, color="red", label="Episodic Cost")
    axes[2].set_xlabel("Episode")
    axes[2].set_ylabel("Total Cost")
    axes[2].set_title("Episodic Cost per Episode")
    axes[2].legend()
...
def main():
    ...
    model_dir = "./models/Swimmer_PD/run1"
    data_dir = "./data_train/Swimmer_PD/run1"
    plot_dir = "./plot_data/Swimmer_PD/run1"
    os.makedirs(model_dir, exist_ok=True)
    os.makedirs(data_dir, exist_ok=True)
    os.makedirs(plot_dir, exist_ok=True)

    # env = gym.make("CartPole-v1")
    env = SwimmerWithPos(sigma_viscosity=0.0, max_steps=1000)
    seed = 2
    ...
    episode_rewards = []
    episode_costs = []
    # Add this list before the training loop:
    lambda_history = []
    episode_max_costs = []
    ...
    for epoch in range(n_epochs):
        ...
        while not done:
            ...
            dist = actor.get_dist(s_tensor)
            # a = dist.sample()
            # print(a)
            a_raw = dist.sample()                          # shape: (1, action_dim)
            log_prob = dist.log_prob(a_raw).sum(dim=-1)    # sum over action dims → (1,)

            # Convert to numpy for env.step()
            a = a_raw.squeeze(0).detach().numpy() 

            s_next, r, c, term, trunc, _ = env.step(a)
            done = term or trunc
        ...
        # Update max cost during the episode
        max_cost = max(costs)  # Get the maximum cost of the episode
        ...
        # Replace the plot call at the bottom of the loop:
        if epoch % 1 == 0:  # Save plot every 50 episodes
            plot_metrics(
                episode_rewards,
                episode_max_costs,
                episode_costs,
                baseline=baseline,
                save=True,
                filename=os.path.join(plot_dir, "training_metrics.png")
            )
