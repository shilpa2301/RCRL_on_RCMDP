...
class Actor_Gaussian(nn.Module):
    ...
    def get_dist(self, s):
        ...
        if torch.isnan(mean).any() or torch.isnan(std).any():
            print("NaNs found in mean or std")
            print("Mean:", mean)
            print("Std:", std)
...
class Robust_RCAC_NPG:
    ...
    def update(self, replay_buffer, total_steps):
        ...
        lambda_ = 0.0
        lr_lambda = 1e-4  # Learning rate for the dual variable
        baseline = self.persistent_eps  # Set the baseline for cost
        ...
        for _ in range(self.K_epochs):
            ...
            with torch.no_grad():  # adv and v_target have no gradient
                ...
                # Calculate deltas for rewards and costs
                deltas_r = r + self.gamma * (1 - dw) * vs_ - vs
                deltas_c = c + self.gamma * (1 - dw) * vcs_ - vcs

                # Calculate advantages using GAE
                for delta_r, d in zip(reversed(deltas_r.flatten().numpy()), reversed(done.flatten().numpy())):
                    gae = delta_r + self.gamma * self.lamda * gae * (1.0 - d)
                    adv_r.insert(0, gae)
                adv_r = torch.tensor(adv_r, dtype=torch.float).view(-1, 1)

                for delta_c, d in zip(reversed(deltas_c.flatten().numpy()), reversed(done.flatten().numpy())):
                    gae = delta_c + self.gamma * self.lamda * gae * (1.0 - d)
                    adv_c.insert(0, gae)
                adv_c = torch.tensor(adv_c, dtype=torch.float).view(-1, 1)
            ...
            for index in BatchSampler(SubsetRandomSampler(range(self.batch_size)), self.mini_batch_size, False):
                ...
                # Calculate Lagrangian objective
                lagrangian = adv_r[index] - lambda_ * adv_c[index]  # Lagrangian objective

                # Calculate surrogate losses
                surr1 = ratios * lagrangian
                surr2 = torch.clamp(ratios, 1 - self.epsilon, 1 + self.epsilon) * lagrangian
                ...
                # Ensure adv_r[index] and vs have compatible shapes
                Rcritic_loss = F.mse_loss(vs[index], adv_r[index].squeeze() + v_s)  # Adjust indexing and shape
                ...
                Ccritic_loss = F.mse_loss(vcs[index], adv_c[index].squeeze() + v_cs)  # Adjust indexing and shape
            ...
            cost_mean = c.mean().item()  # Calculate the average cost
            if total_steps > 200:  # Start updating the dual variable after some steps
        ...
        if self.adaptive_alpha:
            alpha_loss = -(
                self.log_alpha.exp() * (a_logprob.sum(dim=1, keepdim=True) + self.target_entropy).detach()
            ).mean()
...
def main(args, run_number):
    ...
    model_dir = f"./models/PD_{args.env}/run{run_number}/"
    data_train_dir = f"./data_train/PD_{args.env}/run{run_number}/"
    plot_data_dir = f"./plot_data/PD_{args.env}/run{run_number}/"
    ...
    writer = SummaryWriter(
        log_dir=f"runs/RNAC/PD_env_{args.env}_{args.policy_dist}_run{run_number}_seed_{seed}_GAMMA_{GAMMA}"
    )
