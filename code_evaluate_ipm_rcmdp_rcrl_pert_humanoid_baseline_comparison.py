...
from envs.humanoid import HumanoidWithCost, HumanoidWithCostPerturbedTest
...
def safe_reset_env(env, seed=None):
    """
    Handle different reset formats safely.

    Possible formats:
        obs
        obs, info
        [[obs]], info or other nested custom formats
    """
    if seed is not None:
    ...
    # Keeps compatibility if reset output is nested.
    while state.ndim > 1:
        state = state[0]
...
def safe_step_env(env, action):
    """
    Handle different step formats safely.
    ...
    if len(step_out) == 6:
        ...
        raise ValueError(
            f"Unsupported env.step output length: {len(step_out)}"
        )
...
def test_agent_single_model(
    ...
    """
    Evaluate one saved Humanoid model.
    """
    ...
    for episode in range(num_episodes):
        ...
        while not done:
            ...
            next_state, reward, cost, done, info = safe_step_env(env, action)
...
def make_eval_env(std):
    """
    Create the Humanoid perturbed evaluation environment.

    If your HumanoidWithCostPerturbed uses a different argument name,
    change only this line:

        HumanoidWithCostPerturbed(sigma_gravity=std)

    Possible alternatives:
        HumanoidWithCostPerturbed(gravity_std=std)
        HumanoidWithCostPerturbed(sigma=std)
        HumanoidWithCostPerturbed(sigma_viscosity=std)
    """
    env = HumanoidWithCostPerturbedTest(sigma_gravity=std)
    return env
...
def plot_evaluation_single(
    ...
    """
    Plot evaluation results for one Humanoid model per method.
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser("Hyperparameters Setting for RNAC Humanoid Evaluation")

    parser.add_argument(
        "--env",
        type=str,
        default="HumanoidWithCost",
        help="HumanoidWithCost/HumanoidWithCostPerturbed"
    )
    ...
    # If your Humanoid threshold is 0.1, keep this.
    # If you want the older Humanoid training threshold, change to 1.0.
    ...
    # Kept for compatibility. Remove or rename if Humanoid does not use it.
    ...
    # ============================================================
    # Single Humanoid evaluation.
    #
    # Known from your Humanoid training setup:
    #   Surrogate Obj: ./models/HumanoidWithCost/run1/Best_RCAC
    #   Ours:          ./models/HumanoidWithCostPerturbed/run1/Best_RCAC
    #
    # Fill/adjust PD/FAC/RCRL/RESPO paths if your folders differ.
    # ============================================================
    model_specs = [
        {
            "label": "Ours",
        ...
        {
            "label": "Surrogate Obj",
            "model_path": "./models/HumanoidWithCost/run1/Best_RCAC"
        },
        {
            "label": "PD",
            "model_path": "./models/HumanoidWithCost/run103/Best_RCAC"
        },
        {
            "label": "FAC",
            "model_path": "./models/HumanoidWithCost/run501/Best_RCAC"
        },
        {
            "label": "RCRL",
        ...
        {
            "label": "RESPO",
            "model_path": "./models/HumanoidWithCost_PD_RESPO/run1/Best_RCAC"
        },
    ]
    ...
    # Change this if your Humanoid perturbation std is different.
    ...
    plot_evaluation_single(
        ...
        perturbation_stds=perturbation_stds,
        save=True,
        base_filename="plot_inference/humanoid_inference_all",
        smooth_window=20,
        save_legends=False
    )
