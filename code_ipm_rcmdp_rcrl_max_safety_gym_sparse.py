...
def get_cost_from_info(info):
    """
    Extract cost from Safety Gymnasium info dict.

    Safety Gymnasium commonly stores costs under info["cost"].
    ...
    if "cost_hazards" in info:
        return float(info["cost_hazards"])

    if "cost_vases" in info:
        return float(info["cost_vases"])

    if "cost_pillars" in info:
        return float(info["cost_pillars"])

    if "cost_gremlins" in info:
        return float(info["cost_gremlins"])
