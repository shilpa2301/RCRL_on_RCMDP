...
from envs.pendulum_v1 import PendulumEnv, PendulumCostEnv
...
class Robust_RCAC_NPG:
  ...
  def update(self, replay_buffer, total_steps):
        s, a, a_logprob_old, r,c, s_, dw, done = replay_buffer.numpy_to_tensor()  # Get training data
        """
        ...
        with torch.no_grad():  # adv and v_target have no gradient
            ...
            if ch==1:
              ...
              deltas = (1-self.gamma)*c + self.gamma *self.log_sum_exp_fn(c, ((1.0 - dw) * vcs_)) - vcs - self.alpha * a_logprob_old.sum(dim=1, keepdim=True) - self.weight_reg * reg_norm
              ...
              v_target = adv + vcs + self.alpha * a_logprob_old.sum(dim=1, keepdim=True) 
              ...
              for layer in self.Rcritic.children():
                  if isinstance(layer, nn.Linear):
                      ...
                      bias_norm.append(torch.norm(layer.state_dict()['bias']) ** 2)
              reg_norm = torch.sqrt(torch.sum(torch.stack(weight_norm)) + torch.sum(torch.stack(bias_norm[0:-1])))
              deltas = r + self.gamma * (1.0 - dw) * vs_ - vs - self.alpha * a_logprob_old.sum(dim=1, keepdim=True) - self.weight_reg * reg_norm
              for delta, d in zip(reversed(deltas.flatten().numpy()), reversed(done.flatten().numpy())):
                    gae = delta +self.gamma * self.lamda * gae * (1.0 - d) 
                    adv.insert(0, gae)
              adv = torch.tensor(adv, dtype=torch.float).view(-1, 1)
              v_target = adv + vs + self.alpha * a_logprob_old.sum(dim=1, keepdim=True)
              if self.use_adv_norm:  # Trick 1:advantage normalization
                  adv = ((adv - adv.mean()) / (adv.std() + 1e-5))
        ...
        for _ in range(self.K_epochs):
            ...
            for index in BatchSampler(SubsetRandomSampler(range(self.batch_size)), self.mini_batch_size, False):
                ...
                ratio = torch.exp(log_probs.sum(1, keepdim=True) - a_logprob_old_sum[index].sim(1, keepdim=True))
...
def main(args, run_number):
    ...
    if args.env == 'CartPolePerturbedEnv':
        ...
        env = PendulumCostEnv() #CartPolePerturbedEnv() #CartPoleCostEnv()#gym.make(args.env)
        env_evaluate = PendulumCostEnv() #CartPolePerturbedEnv() # CartPoleCostEnv()#gym.make(args.env)  # When evaluating the policy, we need to rebuild an environment
    ...
    for total_steps in tqdm(range(args.max_train_steps)):
        ...
        while not done:
            ...
            replay_buffer.store(s, a, a_logprob+reward_offset, r,c, s_, dw, done)
            s = copy.deepcopy(s_)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--env", type=str, default='PendulumEnv',help="HopperPerturbed/CartPolePerturbedEnv/CartPoleCostEnv")
