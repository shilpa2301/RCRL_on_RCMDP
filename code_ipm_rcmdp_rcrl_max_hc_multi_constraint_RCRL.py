...
class RCRL:
    def __init__(self, args):
        ...
        # ============================================================
        # RCRL multi-constraint dual variables
        # ============================================================
        self.cost_dim = args.cost_dim

        self.dual_lambda = torch.zeros(
            self.cost_dim,
            dtype=torch.float32,
        )

        self.dual_lr = getattr(args, "dual_lr", 1e-3)
        self.dual_lambda_max = getattr(args, "dual_lambda_max", 100.0)
    ...
    def update(self, replay_buffer, total_steps):
        ...
        if not isinstance(self.dual_lambda, torch.Tensor):
            ...
            self.dual_lambda = self.dual_lambda.to(device)

        if self.dual_lambda.ndim == 0:
            self.dual_lambda = torch.zeros(
                self.cost_dim,
                dtype=torch.float32,
                device=device,
        ...
        if isinstance(self.persistent_eps, (list, tuple, np.ndarray)):
        ...
        for _ in range(self.K_epochs):
            ...
            with torch.no_grad():
                # ====================================================
                # Reward and cost critic predictions
                # ====================================================
                ...
                # ====================================================
                # Multi-constraint RCRL peak-value estimate
                # ====================================================
                # Peak robust cost per constraint
                # shape: [cost_dim]
                vl_pi_each_cost = vcs.max(dim=0).values

                # Constraint violation per constraint
                # shape: [cost_dim]
                constraint_violation_each = vl_pi_each_cost - eps_vec

                # Scalar summaries for logging
                vl_pi = vl_pi_each_cost.max()
                worst_violation = constraint_violation_each.max()

                # ====================================================
                # Vector dual update
                #
                # lambda_i <- [lambda_i + dual_lr * (V_C_i - eps_i)]_+
                # ====================================================
                if self.warm_start_flag == 1:
                    self.dual_lambda = self.dual_lambda + self.dual_lr * constraint_violation_each
                ...
                print(
                    "RCRL-Multi | "
                    f"lambda={self.dual_lambda.detach().cpu().numpy()}, "
                    f"eps={eps_vec.detach().cpu().numpy()}, "
                    f"vl_each={vl_pi_each_cost.detach().cpu().numpy()}, "
                    f"violation_each={constraint_violation_each.detach().cpu().numpy()}, "
                    f"vl_max={vl_pi.item():.6f}, "
                    f"worst_violation={worst_violation.item():.6f}"
                )

                # ====================================================
                # Cost critic target
                #
                # RCRL robust/persistent style:
                # V_C(s) approximates peak future cost.
                # Backup uses smooth max between immediate cost and next value.
                # ====================================================

                # Make dw broadcastable from [B, 1] to [B, cost_dim]
                not_dw = 1.0 - dw
                if not_dw.shape != vcs_.shape:
                    not_dw = not_dw.expand_as(vcs_)

                # Next bootstrapped cost values
                # shape: [B, cost_dim]
                bootstrapped_cost = not_dw * vcs_

                # Elementwise smooth max:
                # cost_backup[:, i] = smooth_max(c[:, i], bootstrapped_cost[:, i])
                # shape: [B, cost_dim]
                cost_backup = self.log_sum_exp_fn(c, bootstrapped_cost)
                ...
                # Last layer regularization for each cost output
                linear_layers_c = [
                ...
                reg_norm_c = torch.norm(last_weight_c, p=2, dim=1)  # [cost_dim]
                reg_norm_c = reg_norm_c.view(1, -1).expand_as(vcs)  # [B, cost_dim]

                # Cost critic target
                # shape: [B, cost_dim]
                v_target_c = cost_deltas + vcs + self.weight_reg * reg_norm_c

                # Cost advantage remains vector-valued
                # shape: [B, cost_dim]
                adv_c = cost_deltas.clone()

                # ====================================================
                # Reward critic target with GAE
                # ====================================================
                linear_layers_r = [
                ...
                for delta, d in zip(
                    reversed(reward_deltas.flatten().cpu().numpy()),
                ...
                if self.use_adv_norm:
                    ...
                    # Normalize each cost dimension separately
                ...
                # ====================================================
                # RCRL primal-dual actor advantage
                #
                # A_RCRL = A_R - sum_i lambda_i A_C_i
                # ====================================================
                lambda_vec = self.dual_lambda.detach().view(1, -1)  # [1, cost_dim]

                cost_penalty_adv = torch.sum(
                    adv_c * lambda_vec,
                    dim=1,
                    keepdim=True,
                )  # [B, 1]

                adv = adv_r - cost_penalty_adv

                if self.use_adv_norm:
                    adv = (adv - adv.mean()) / (adv.std() + 1e-5)

            # ========================================================
            # PPO actor/critic updates
            # ========================================================
...
def save_agent(agent, save_path, ep=0, state_norm=None, reward_scaling=None):
    ...
    # Save RCRL dual variables
    np.save(
        f"{save_path}_dual_lambda_{str(ep)}.npy",
        agent.dual_lambda.detach().cpu().numpy(),
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        default=1e-3,
        help="Learning rate for RCRL dual variables",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=100.0,
        help="Maximum value for each RCRL dual variable",
    )
