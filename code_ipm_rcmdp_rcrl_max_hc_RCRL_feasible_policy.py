...
class RCRL:
    def __init__(self, args):
        if args.env == "HalfCheetahWithPos":
...
def save_agent(agent, save_path, total_steps=None, state_norm=None, reward_scaling=None):
    """
    Save agent in two ways:

    1. Always save/overwrite current best:
        {save_path}_actor
        {save_path}_Rcritic
        {save_path}_Ccritic

    2. If total_steps is provided, also save numbered intermediate checkpoint:
        {save_path}_actor_{total_steps}
        {save_path}_Rcritic_{total_steps}
        {save_path}_Ccritic_{total_steps}
    """

    # ------------------------------------------------------------
    # 1. Save current best, overwritten each time
    # ------------------------------------------------------------
    agent.actor.save(f"{save_path}_actor")
    agent.Rcritic.save(f"{save_path}_Rcritic")
    agent.Ccritic.save(f"{save_path}_Ccritic")
    ...
    # ------------------------------------------------------------
    # 2. Save numbered intermediate feasible best checkpoint
    # ------------------------------------------------------------
    if total_steps is not None:
        agent.actor.save(f"{save_path}_actor_{total_steps}")
        agent.Rcritic.save(f"{save_path}_Rcritic_{total_steps}")
        agent.Ccritic.save(f"{save_path}_Ccritic_{total_steps}")

        if state_norm:
            with open(f"{save_path}_state_norm_{total_steps}", "wb") as file1:
                pickle.dump(state_norm, file1)

        if reward_scaling:
            with open(f"{save_path}_reward_scaling_{total_steps}", "wb") as file2:
                pickle.dump(reward_scaling, file2)

        print(
            f"[SAVE] Saved current best and numbered checkpoint at step/episode {total_steps}: "
            f"{save_path}_actor and {save_path}_actor_{total_steps}"
        )
    else:
        print(f"[SAVE] Saved current best checkpoint: {save_path}_actor")
...
#shilpa feasible policy
def load_initial_policy(agent, args):
    """
    Initialize current training run from a previously dumped policy/checkpoint.

    If args.init_model_num is None:
        loads:
            {args.init_policy_path}_actor
            {args.init_policy_path}_Rcritic      optional
            {args.init_policy_path}_Ccritic      optional

    If args.init_model_num is not None:
        loads:
            {args.init_policy_path}_actor_{args.init_model_num}
            {args.init_policy_path}_Rcritic_{args.init_model_num}   optional
            {args.init_policy_path}_Ccritic_{args.init_model_num}   optional

    Example:
        init_policy_path = ./models/HalfCheetahWithPos/run1/Best_RCAC
        init_model_num   = 8000

        actor file:
        ./models/HalfCheetahWithPos/run1/Best_RCAC_actor_8000
    """

    if args.init_policy_path is None:
        return

    if args.init_model_num is None:
        actor_path = f"{args.init_policy_path}_actor"
        rcritic_path = f"{args.init_policy_path}_Rcritic"
        ccritic_path = f"{args.init_policy_path}_Ccritic"
    else:
        actor_path = f"{args.init_policy_path}_actor_{args.init_model_num}"
        rcritic_path = f"{args.init_policy_path}_Rcritic_{args.init_model_num}"
        ccritic_path = f"{args.init_policy_path}_Ccritic_{args.init_model_num}"

    if not os.path.exists(actor_path):
        raise FileNotFoundError(f"Initial actor checkpoint not found: {actor_path}")

    print("\n============================================================")
    print("[INIT] Loading feasible initial policy")
    print(f"[INIT] actor path: {actor_path}")
    print("============================================================\n")

    agent.actor.load(actor_path, device="cpu")

    if args.init_load_critics:
        if os.path.exists(rcritic_path):
            print(f"[INIT] Loading reward critic: {rcritic_path}")
            agent.Rcritic.load(rcritic_path, device="cpu")
        else:
            print(f"[WARNING] Rcritic file not found: {rcritic_path}")

        if os.path.exists(ccritic_path):
            print(f"[INIT] Loading cost critic: {ccritic_path}")
            agent.Ccritic.load(ccritic_path, device="cpu")
        else:
            print(f"[WARNING] Ccritic file not found: {ccritic_path}")

    print("[INIT] Finished loading initial policy.\n")
...
def main(args, run_number):
    ...
    # New folder for trajectory/rollout step counts
    step_data_dir = f"./step_data/{args.env}/run{run_number}/"
    ...
    agent = RCRL(args)
    #shilpa feasible policy
    load_initial_policy(agent, args)
    ...
    # New: rollout length tracking
    episode_step_counts = []
    cumulative_step_counts = []
    ...
    for total_steps in tqdm(range(args.max_train_steps)):
        ...
        if total_steps % args.evaluate_freq == 0:
            ...
            if (
                ...
                print(
                    f"New feasible best model found! "
                    f"Saving model with reward: {evaluate_reward:.3f}, "
                    f"max cost: {evaluate_max_cost:.3f}, "
                    f"episode/checkpoint: {total_steps}"
                )

                # Save both:
                #   Best_RCAC_actor
                #   Best_RCAC_actor_<total_steps>
                if args.use_reward_scaling and args.use_state_norm:
                    save_agent(
                        agent,
                        best_model_path,
        ...
        # ------------------------------------------------------------
        # New: save rollout/trajectory step counts
        # ------------------------------------------------------------
        episode_step_counts.append(episode_steps)
        cumulative_env_steps += episode_steps
        cumulative_step_counts.append(cumulative_env_steps)

        np.save(
            f"{step_data_dir}/episode_step_counts.npy",
            np.array(episode_step_counts, dtype=np.int64),
        )

        np.save(
            f"{step_data_dir}/cumulative_step_counts.npy",
            np.array(cumulative_step_counts, dtype=np.int64),
        )

        # Also save a readable CSV file
        step_log = np.column_stack(
            (
                np.arange(len(episode_step_counts), dtype=np.int64),
                np.array(episode_step_counts, dtype=np.int64),
                np.array(cumulative_step_counts, dtype=np.int64),
            )
        )
        ...
        np.savetxt(
            f"{step_data_dir}/step_log.csv",
            step_log,
            delimiter=",",
            header="episode,episode_steps,cumulative_steps",
            comments="",
            fmt="%d",
        )

        print(
            f"[STEP LOG] episode={total_steps}, "
            f"episode_steps={episode_steps}, "
            f"cumulative_env_steps={cumulative_env_steps}"
        )

        # Existing plot/data saves
    ...
    np.save(
        f"{step_data_dir}/final_episode_step_counts.npy",
        np.array(episode_step_counts, dtype=np.int64),
    )

    np.save(
        f"{step_data_dir}/final_cumulative_step_counts.npy",
        np.array(cumulative_step_counts, dtype=np.int64),
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Base checkpoint path without _actor suffix, e.g. ./models/HalfCheetahWithPos/run1/Best_RCAC",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Checkpoint number. If set, loads {init_policy_path}_actor_{init_model_num}. If None, loads {init_policy_path}_actor.",
    )
