...
class LambdaNet(nn.Module):
    ...
    def forward(self, s):
        ...
        # softplus guarantees lambda(s) >= 0
...
class FAC:
    def __init__(self, args):
        if args.env == "CartPolePerturbedEnv":
            self.env = CartPolePerturbedEnv(args.gravity_std)
        ...
        self.lr_lam = getattr(args, "lr_lam", 1e-4)
        ...
        # Important: total_steps in your code is already episode/update index.
        self.warm_start_flag = args.warm_start_flag
        self.warm_start_episode = args.warm_start_episode

        # LambdaNet starts only after warm start by default.
        ...
        if self.set_adam_eps:
            ...
            self.optimizer_Lam = torch.optim.Adam(
                self.Lam.parameters(), lr=self.lr_lam, eps=1e-5
            )
        else:
            self.optimizer_actor = torch.optim.Adam(
            ...
            self.optimizer_Lam = torch.optim.Adam(
                self.Lam.parameters(), lr=self.lr_lam
            )
    ...
    def update(self, replay_buffer, total_steps):
        ...
        # In your code, total_steps is already episode/update index.
        warm_start_active = (
        ...
        if d_threshold.dim() == 0:
            d_threshold = d_threshold.view(1, 1).expand(1, c.shape[1])
        ...
        for _ in range(self.K_epochs):
            ...
            with torch.no_grad():
                ...
                # ============================================================
                # Old diagnostic / warm-start switch
                # ============================================================
                vl_pi = vcs.max()

                penalty_term = vl_pi - torch.tensor(
                    self.persistent_eps,
                    dtype=torch.float32,
                    device=s.device,
                )
                ...
                # ============================================================
                # Cost critic target: kept same as your previous code
                # ============================================================
                linear_layers = [
                    l for l in self.Ccritic.children()
                ...
                C_reg_norm = torch.norm(linear_layers[-1].weight, p=2)
                ...
                cost_deltas = (
                    (1.0 - self.gamma) * c
                    + self.gamma * self.log_sum_exp_fn(c, not_dw * vcs_)
                    - vcs
                )

                adv_c = cost_deltas.clone()
                v_target_c = cost_deltas + vcs + self.weight_reg * C_reg_norm

                # ============================================================
                # Reward critic target: kept same as your previous code
                # ============================================================
                linear_layers = [
                ...
                R_reg_norm = torch.norm(linear_layers[-1].weight, p=2)
                ...
                v_target_r = adv_r + vs + self.weight_reg * R_reg_norm
                ...
                if warm_start_active:
                    # --------------------------------------------------------
                    # Keep your previous warm-start behavior exactly:
                    # ch == 0: reward learning
                    # ch == 1: cost recovery
                    # --------------------------------------------------------
                    ...
                    # --------------------------------------------------------
                    # FAC after warm start:
                    # adv = reward advantage - lambda(s) * cost advantage
                    # --------------------------------------------------------
                    lambda_s = self.Lam(s)
                    lambda_s_detached = lambda_s.detach()

                    if adv_c.dim() == 1:
                        adv_c_for_actor = adv_c.view(-1, 1)
                    else:
                        adv_c_for_actor = adv_c

                    weighted_cost_adv = torch.sum(
                        lambda_s_detached * adv_c_for_actor,
                        dim=1,
                        keepdim=True,
                    )
                ...
                print(
                    ...
                    f"vs_mean={vs_mean:.4f}, "
                    "vl_pi=",
                    vl_pi.detach().cpu().numpy(),
                    "beta_penalty=",
                    beta_penalty.detach().cpu().numpy(),
                    "lambda_mean=",
                    lambda_mean.detach().cpu().numpy(),
                    "VC_mean=",
                    vcs.mean(dim=0).detach().cpu().numpy(),
                    "d=",
                    d_threshold.view(-1).detach().cpu().numpy(),
                )
            ...
            for index in BatchSampler(
                ...
                if allow_lambda_update:
                    ...
                    # Gradient ascent on lambda objective:
                    # maximize lambda(s) * (V_C(s) - d)
                    # optimizer minimizes, so use negative.
                    ...
                    if self.use_grad_clip:
                        torch.nn.utils.clip_grad_norm_(self.Lam.parameters(), 0.5)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Update lambda network every lambda_delay training steps",
    )

    parser.add_argument("--lambda_start_episode", type=int, default=500)
    ...
    parser.add_argument(
        ...
        type=bool,
        default=False,
        help="Only update LambdaNet when V_C exceeds threshold",
    )
