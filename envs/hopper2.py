...
class HopperPerturbedEnv(MujocoEnv, EzPickle):
    ...
    def step(self, action):
        ...
        z, angle = self.data.qpos[1:3]
        cost = self.compute_cost(z, angle)
    ...
    def compute_cost(self, z, angle):
        ...
        c_h = max(0.0, (h_safe - z) / delta_h)

        # Compute the angle cost
        c_theta = max(0.0, (abs(angle) - theta_max) / delta_theta)

        # Total cost is the sum of the two components
        cost = c_h + c_theta
        # # Add a penalty if the hopper becomes unhealthy (falls or tilts too much)
        # if not self.is_healthy:
        #     cost += 10.0 #0.0 #0.5 #10.0  # Penalty for becoming unhealthy
