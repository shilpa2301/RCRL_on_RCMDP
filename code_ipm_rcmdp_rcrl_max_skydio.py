...
class RPCRL:
    def __init__(self, args):
        ...
        elif args.env == "SkydioTrackingEnv":
            self.env = SkydioTrackingEnv()
        else:
...
def main(args, run_number):
    ...
    if args.env == "CartPolePerturbedEnv":
        ...
        env_evaluate = SkydioTrackingEnv()
        env_reset = SkydioTrackingEnv()
    ...
    for total_steps in tqdm(range(args.max_train_steps)):
        ...
        s = env.reset()[0]#[0]
        # print("Initial state:", s.shape)  # Debugging: Print the initial state
